...
def gradient_desent(X, Y, alpha, iters):
	n = X.shape[0]
	...
	for it in range(iters):
		pred = np.matmul(X, W)
		#print(W.T)
		W = W - (1/n) * alpha * (np.matmul(X.T, (pred - Y)))
		#print(W.T)
		W_tt[:,[it]] = W
...
#print(W.T)
print(W_ML.T)
W_t, W_tt = gradient_desent(X, Y, 0.0001, 5000)
print(W_t.T)
...
print(diff_in_w(W_ML.flatten(), W_t.flatten()))

plt.plot([i for i in range(5000)], Errors)
plt.xlabel("Time t")
